...
class LoopDetection:
    ...
    def __init__(self, loop_type: str, pattern: str, confidence: float,
                 occurrences: int, first_occurrence: datetime,
...
class MoodAnalyzer:
    ...
    def get_loop_detection_summary(self, character_id: str) -> Dict[str, Any]:
        ...
        recent_loops = [
            ...
            if loop.last_occurrence > recent_cutoff
        ]
        
        # High confidence loops
        high_confidence_loops = [
            loop for loop in loops 
    ...
    def _extract_emotional_indicators(self, text: str) -> List[Dict[str, Any]]:
        ...
        emotion_patterns = {
            'happiness': ['happy', 'joy', 'pleased', 'delighted', 'cheerful', 'smile', 'laugh'],
    ...
    def _clean_old_patterns(self, character_id: str) -> None:
        ...
        self.detected_loops[character_id] = [
            loop for loop in loops 
            if loop.last_occurrence > cutoff_time
        ]
    ...
    def _detect_emotional_state_loops(self, character_id: str, 
                                    current_indicators: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _calculate_mood_stability(self, patterns: List[Dict[str, Any]]) -> float:
        ...
        for pattern in patterns:
            if pattern['emotional_indicators']:
                avg_intensity = sum(
                    indicator['intensity'] for indicator in pattern['emotional_indicators']
        ...
        return {
            'mood_analyzer': {
                ...
                'total_dialogue_entries': sum(
                    len(dialogue) for dialogue in self.character_dialogue_history.values()
                ),
                'total_patterns': sum(
                    len(patterns) for patterns in self.character_emotional_patterns.values()
                ),
