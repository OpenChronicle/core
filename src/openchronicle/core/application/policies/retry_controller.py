...
class RetryController:
    ...
    def should_retry(
        ...
        Logic:
        1. If prior_attempts >= policy.max_attempts - 1, we've exhausted attempts: return False
        2. If policy.retry_on_errors is set and error_code not in list: return False
        3. Otherwise: return True
        """
        # Attempt 1 fails: prior_attempts = 1, need prior_attempts < max_attempts
        # Attempt 2 fails: prior_attempts = 2, need prior_attempts < max_attempts
        # So: check if we've already used our allotted attempts
