...
from ..shared import BookmarkManagerException, BookmarkType
...
class NavigationManager:
    ...
    def _generate_quick_links(self, timeline: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        # Add first bookmark
        if timeline:
            quick_links.append({"type": "first", "label": "Beginning", "bookmark": timeline[0]})
        ...
        for chapter in chapter_bookmarks[-5:]:  # Last 5 chapters
            quick_links.append({"type": "chapter", "label": chapter["label"], "bookmark": chapter})

        # Add last bookmark
        if timeline and len(timeline) > 1:
            quick_links.append({"type": "last", "label": "Latest", "bookmark": timeline[-1]})
