...
from .engines.caching.config import ClusterNode, DistributedCacheConfig, PartitionConfig
...
class DistributedMultiTierCache(MultiTierCache):
    ...
    def __init__(self, config: DistributedCacheConfig | None = None):
        ...
        if self.config.enable_local_cache and CACHETOOLS_AVAILABLE:
            self.local_cache = TTLCache(maxsize=self.config.local_cache_size, ttl=300)  # 5 minutes local TTL
        else:
