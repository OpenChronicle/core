...
- [ ] FastAPI app factory with lifespan container injection
- [ ] `HTTPConfig` dataclass (host, port, auth settings), three-layer precedence
- [ ] Authentication middleware (API key / token)
- [ ] Privacy gate middleware (PII detection on inbound)
- [ ] Per-client rate limiting middleware
- [ ] Core routes mirroring MCP surface:
  - [ ] `/api/v1/memory/*` (search, save, list, pin)
  - [ ] `/api/v1/conversation/*` (ask, history, list, create)
  - [ ] `/api/v1/project/*` (create, list)
  - [ ] `/api/v1/asset/*` (upload, list, get, link)
...
- [ ] Wire HTTP server startup into `oc serve`
...
5. HTTP API (P6) ← MOVED UP (always-on daemon infrastructure, Decision #6)
   └── Core interface in interfaces/api/
   └── FastAPI, mirrors MCP tools as REST endpoints
   └── Starts with oc serve, core dependency (not optional)
   └── Unblocks: webhooks, plugin route registration, external integrations
