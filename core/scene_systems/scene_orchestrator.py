...
def generate_scene_id() -> str:
    """Backward compatibility function for scene ID generation."""
    generator = SceneIdGenerator()
    return generator.generate_scene_id()

def save_scene(story_id: str, user_input: str, model_output: str, 
               memory_snapshot=None, flags=None, context_refs=None, 
               analysis_data=None, scene_label=None, token_manager=None, 
               model_name=None, structured_tags=None) -> str:
    """Backward compatibility function for scene saving."""
    orchestrator = SceneOrchestrator(story_id)
    return orchestrator.save_scene(
        user_input, model_output, memory_snapshot, flags, context_refs,
        analysis_data, scene_label, token_manager, model_name, structured_tags
    )

def load_scene(story_id: str, scene_id: str) -> Optional[Dict[str, Any]]:
    """Backward compatibility function for scene loading."""
    orchestrator = SceneOrchestrator(story_id)
    return orchestrator.load_scene(scene_id)

def list_scenes(story_id: str) -> List[Dict[str, Any]]:
    """Backward compatibility function for scene listing."""
    orchestrator = SceneOrchestrator(story_id)
    return orchestrator.list_scenes()

def get_scenes_with_long_turns(story_id: str) -> List[Dict[str, Any]]:
    """Backward compatibility function."""
    orchestrator = SceneOrchestrator(story_id)
    return orchestrator.get_scenes_with_long_turns()

def get_scenes_by_mood(story_id: str, mood: str) -> List[Dict[str, Any]]:
    """Backward compatibility function."""
    orchestrator = SceneOrchestrator(story_id)
    return orchestrator.get_scenes_by_mood(mood)

def get_scenes_by_type(story_id: str, scene_type: str) -> List[Dict[str, Any]]:
    """Backward compatibility function."""
    orchestrator = SceneOrchestrator(story_id)
    return orchestrator.get_scenes_by_type(scene_type)

def get_token_usage_stats(story_id: str) -> Dict[str, Any]:
    """Backward compatibility function."""
    orchestrator = SceneOrchestrator(story_id)
    return orchestrator.get_token_usage_stats()

def get_character_mood_timeline(story_id: str, character_name: str) -> List[Dict[str, Any]]:
    """Backward compatibility function."""
    orchestrator = SceneOrchestrator(story_id)
    return orchestrator.get_character_mood_timeline(character_name)

def update_scene_label(story_id: str, scene_id: str, scene_label: str) -> bool:
    """Backward compatibility function."""
    orchestrator = SceneOrchestrator(story_id)
    return orchestrator.update_scene_label(scene_id, scene_label)

def get_scenes_by_label(story_id: str, scene_label: str) -> List[Dict[str, Any]]:
    """Backward compatibility function."""
    orchestrator = SceneOrchestrator(story_id)
    return orchestrator.get_scenes_by_label(scene_label)

def get_labeled_scenes(story_id: str) -> List[Dict[str, Any]]:
    """Backward compatibility function."""
    orchestrator = SceneOrchestrator(story_id)
    return orchestrator.get_labeled_scenes()

def rollback_to_scene(story_id: str, scene_id: str) -> bool:
    """Backward compatibility function."""
    orchestrator = SceneOrchestrator(story_id)
    return orchestrator.rollback_to_scene(scene_id)

def get_scene_summary_stats(story_id: str) -> Dict[str, Any]:
    """Backward compatibility function."""
    orchestrator = SceneOrchestrator(story_id)
    return orchestrator.get_scene_summary_stats()
