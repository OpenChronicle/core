...
class SceneSerializer:
    ...
    def serialize_scene_summary(self, scene_data: SceneData) -> Dict[str, Any]:
        ...
        return {
            ...
            "timestamp": scene_data.timestamp,
            "scene_label": scene_data.scene_label,
            "input_preview": scene_data.user_input[:100] + "..." if len(scene_data.user_input) > 100 else scene_data.user_input,
            "output_preview": scene_data.model_output[:100] + "..." if len(scene_data.model_output) > 100 else scene_data.model_output,
    ...
    def deserialize_from_export(self, export_data: Dict[str, Any]) -> List[SceneData]:
        ...
        for scene_dict in export_data.get("scenes", []):
            try:
                ...
                print(f"Error deserializing scene {scene_dict.get('scene_id', 'unknown')}: {e}")
                continue
